...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        Log.d("asdfasdf645456",words.toString())
        if (words.isNotEmpty()) {
        ...
        {
            Toast.makeText(applicationContext, "add new word from menu", Toast.LENGTH_SHORT).show()
        }
    }
    ...
    override fun onOptionsItemSelected(item: MenuItem): Boolean {
        when(item.itemId){
            R.id.addNew->{
                startActivity(Intent(applicationContext,AddWords::class.java))
                return true
            }
        }
